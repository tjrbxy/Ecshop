...
class cls_template
{
    ...
    function fetch_str($source)
    {
        ...
        if(preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $source, $sp_match))
        {
         ...
         return preg_replace_callback("/{([^\}\{\n]*)}/",function ($res){ return $this->select($res[1]);},$source);
...
//            }
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $out = "<?php \n" . '$k = ' . preg_replace_callback("/(\'\\$[^,]+)/" , function ($res){
                        return stripslashes(trim($res[1],'\''));
                        }, var_export($t, true)) . ";\n";
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            $source      = preg_replace_callback($pattern,function ($res){return "{include file='".strtolower($res[1])."'}"; },$source);
            /* 检查有无动态库文件，如果有为其赋值 */
